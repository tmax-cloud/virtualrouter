...
type NATRuleLister interface {
	...
	List(selector labels.Selector) (ret []*v1.NATRule, err error)
...
// List lists all NATRules in the indexer.
func (s *nATRuleLister) List(selector labels.Selector) (ret []*v1.NATRule, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.NATRule))
	})
	return ret, err
}
...
// List lists all NATRules in the indexer for a given namespace.
func (s nATRuleNamespaceLister) List(selector labels.Selector) (ret []*v1.NATRule, err error) {
...
// Get retrieves the NATRule from the indexer for a given namespace and name.
func (s nATRuleNamespaceLister) Get(name string) (*v1.NATRule, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("natrule"), name)
	}
	return obj.(*v1.NATRule), nil
}
